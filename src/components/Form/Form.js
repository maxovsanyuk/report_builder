...
export default class Form extends Component {
  getForm = (formFields) => {
    ...
    if (this.props.formData) {
      ...
      formFields.forEach((field) => {
        if (field.type === "additionalSection") {
          //assigning fields from additional sections
          switch (field.name) {
  ...
  validate = () => {
    ...
    this.props.formFields
      .filter((field) => !field.hidden && field.validation?.length)
      .forEach((field) => {
            ...
            case "required":
              if (field.type === "text") {
                validatorsValue.push(
                  !this.state.form[field.name] ? "required" : false
                );
              }
              if (field.type === "optionSet") {
                validatorsValue.push(
                  !this.state.form[field.name].value ? "required" : false
                );
              }
              break;
            case "minLength":
              validatorsValue.push(
                this.state.form[field.name].length < validator.value
                  ? "minLength"
                  : false
              );
              break;
        ...
        errors[field.name] = validatorsValue.some((value) => value)
          ? validatorsValue.find((value) => value)
          : false;
      });
    ...
    return (
      <Fragment>
              ...
              return (
                <FormControlLabel
                  ...
                  control={
                    <Checkbox
                      onChange={(event) => this.fieldChanged(event, field)}
                      checked={this.state.form[field.name]}
                      name={field.name}
                    />
                  }
                  label={field.label}
              ...
              return (
                <Fragment key={field.name}>
                  ...
                  <Tabs
                    ...
                    onChange={(e, newValue) =>
                      this.tabsHandleChange("valueType", newValue)
                    }
                  ...
                  <TabPanel value={this.state.form.valueType} index={0}>
                    ...
                    >
                      <div style={{ minHeight: "250px" }}>
                        ...
                        <button>Add</button>
                        <div
                          style={{ border: "1px solid #000", height: "200px" }}
                        />
                      </div>
                    </TabPanel>
                    <TabPanel
                    ...
                    >
                      <div>
                        <h4>
                          Choose the dataset and fields for the available
                          values:
                        </h4>
                        <FormControl className={classes.formControl}>
                          <InputLabel>Dataset</InputLabel>
                          <Select
                            value={this.state.form.availableQueryDataset}
                            onChange={(event) =>
                              this.fieldChanged(event, {
                                type: "optionSet",
                                name: "availableQueryDataset",
                              })
                            }
                          >
                        ...
                        <FormControl className={classes.formControl}>
                          ...
                          <Select
                            value={this.state.form.availableQueryField}
                            onChange={(event) =>
                              this.fieldChanged(event, {
                                type: "optionSet",
                                name: "availableQueryField",
                              })
                            }
                          >
                            {this.props.dataSets
                              .find(
                                (dataSet) =>
                                  dataSet.id ===
                                  this.state.form.availableQueryDataset
                              )
                              ?.fields.map((field) => (
        ...
        <div className={classes.buttonsWrapper}>
          <Button
            onClick={() => this.props.save(this.state.form, this.state.mode)}
