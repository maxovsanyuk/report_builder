...
export default class SidebarParameters extends Component {
  ...
  submitParameter = (parameter, mode) => {
    ...
    switch (mode) {
      case 'save':
        ...
        break;
      case 'edit':
        let editParameterIndex = parameters.findIndex(param => param.id === parameter.id);
  ...
  newParameter = () => {
    const newFormFields = this.state.formFields.filter(field => field.name !== 'id');
    newFormFields.push({
      name: 'id',
      type: 'text',
      label: 'Id',
  ...
  deleteParameter = (parameterId) => {
    const parameters = cloneDeep(this.props.parameters).filter(parameter => parameter.id !== parameterId);
    this.props.setReportParameters(parameters);
  };
    ...
    return (
      <Fragment>
        ...
        {this.state.isForm
          ...
          : <SidebarParametersList onDeleteParameter={this.deleteParameter}
